...
class PhotoView: UIView {
    ...
    @objc var picArr = [UIImage]() {
        didSet{
            ...
            let magin: CGFloat = 12
            let imageW: CGFloat = (screenWidth-5*magin)/4 + 2
            let imageH: CGFloat = 110
            ...
            for i in 0..<picArr.count {
                ...
                // 加一个关闭按钮
                let closeBtn = UIButton.init(frame: CGRect(x: self.imgView.frame.size.width - 20, y: 0, width: 20, height: 20))
                closeBtn.tag = i
                if (i != picArr.count-1) {
