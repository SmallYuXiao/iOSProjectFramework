...
extension UIImage {
    ...
    public func croppedImage(_ bound: CGRect) -> UIImage? {
        guard self.size.width > bound.origin.x else {
            printLog("EZSE: Your cropping X coordinate is larger than the image width")
            return nil
        }
        guard self.size.height > bound.origin.y else {
            printLog("EZSE: Your cropping Y coordinate is larger than the image height")
            return nil
        }
    ...
    public convenience init?(urlString: String) {
        ...
        guard let data = try? Data(contentsOf: url) else {
            printLog("EZSE: No image in URL \(urlString)")
            self.init(data: Data())
            return
        }
        self.init(data: data)
